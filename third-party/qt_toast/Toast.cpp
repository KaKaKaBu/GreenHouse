...
QPoint Toast::calculatePosition()
{
    ...
    // Check if we should use parent window geometry instead of screen geometry
    bool useParentGeometry = m_parent && !s_alwaysOnMainScreen && !s_fixedScreen;

    // Calculate x and y position of notification
    int x = 0;
    int y = 0;

    if (useParentGeometry)
    {
        // Use parent window geometry for positioning
        QRect parentGeometry = m_parent->geometry();

        if (s_position == ToastPosition::BOTTOM_RIGHT)
        {
            x = parentGeometry.x() + parentGeometry.width() - m_notification->width() - s_offsetX;
            y = parentGeometry.y() + parentGeometry.height() - m_notification->height() - s_offsetY - offsetY;
        }
        else if (s_position == ToastPosition::BOTTOM_LEFT)
        {
            x = parentGeometry.x() + s_offsetX;
            y = parentGeometry.y() + parentGeometry.height() - m_notification->height() - s_offsetY - offsetY;
        }
        else if (s_position == ToastPosition::BOTTOM_MIDDLE)
        {
            x = parentGeometry.x() + (int)(parentGeometry.width() / 2 - m_notification->width() / 2);
            y = parentGeometry.y() + parentGeometry.height() - m_notification->height() - s_offsetY - offsetY;
        }
        else if (s_position == ToastPosition::TOP_RIGHT)
        {
            x = parentGeometry.x() + parentGeometry.width() - m_notification->width() - s_offsetX;
            y = parentGeometry.y() + s_offsetY + offsetY;
        }
        ...
        {
            x = parentGeometry.x() + (int)(parentGeometry.width() / 2 - m_notification->width() / 2);
            y = parentGeometry.y() + (int)(parentGeometry.height() / 2 - m_notification->height() / 2 + offsetY);
        }
    }
    else
    {
        // Use screen geometry for positioning (original behavior)
